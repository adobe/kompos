...
### Added
- **`komposconfig.compositions.build_order`** — list of `{runner, type}` pairs that controls cross-runner dispatch order in `compile build`; enables interleaving like `external/ipam → tfe/cell → tfe/cluster` that runner-level `order` cannot express.
